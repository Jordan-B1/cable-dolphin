...
static void display_flags(const uint8_t flags, output_buffer_t *output_buffer) {
  char flags_name[NB_TCP_FLAGS][4] = {"CWR", "ECE", "URG", "ACK",
                                      "PSH", "RST", "SYN", "FIN"};

  write_buffer(output_buffer, "Flags:");
  for (int i = 0; i < NB_TCP_FLAGS; i++) {
    if ((flags >> i) & 0x01) {
      write_buffer(output_buffer, " %s", flags_name[i]);
    }
  }
...
bool display_tcp_packet_info(const uint8_t *packet, const size_t packet_len,
  ...
  write_buffer(output_buffer, "Info: SEQ: %u ACK: %u\n", header.sequence_number,
               header.ack_number);
  display_flags(header.flags, output_buffer);
  return true;
}